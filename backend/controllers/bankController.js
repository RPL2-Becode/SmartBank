...
const generateTxId = () => `TX-${Date.now()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
...
exports.transfer = async (req, res) => {
    ...
    try {
        ...
        const { toUserId, amount } = req.validatedData || req.body;
...
exports.payment = async (req, res) => {
    ...
    try {
        const fromUserId = req.user.userId;
        const { toUserId, amount, type, description } = req.validatedData || req.body;
...
exports.requestLoan = async (req, res) => {
    ...
    try {
        const userId = req.user.userId;
        const { amount } = req.validatedData || req.body;
